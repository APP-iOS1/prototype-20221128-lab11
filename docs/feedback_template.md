...
- 저희 앱은 특정한 장소에서 들었던 음악을 기록하기 위한 앱입니다. 앱을 사용하면 그 목적이 잘 확인되나요?
1. 목적이 직관적으로 다가오지 않음. 장소라는 키워드는 좋음
2.전체적으로 잘 확인이 됨
- 화면에 나타나는 요소들은 어떤 의미인지 보이나요?
    - 헷갈리거나 불편한 UI가 있나요? 있으면 이유를 함께 적어주세요
    ...
    - 앱을 사용하면서 마음에 들었던 부분이 있나요? 있으면 이유를 함께 적어주세요.
        - 맵에 롱프레스를 눌러서 추가할 수 있는 부분이 설명만 있다면 매우 좋았습니다.
        - 사진이 아닌, 음악으로 장소를 기억한다는 아이디어가 매우 참신합니다.
        but 핵심기능이 숨겨져 있는 듯함 --> 핵심 기능 부각
- 사용하면서 개선이 필요한 부분이 있나요?
nearby를 홈부분으로 채택해도 좋을거같습니다
이동 중일 때 어떻게 처리를 할 건지(동선)
비슷한 장소는 마커가 하나로 뭉치게끔
음악카테고리를 채택하셨는데, 일단 홈부터 지도가 보여서 음악카테고리에 좀 맞지않는 느낌입니다.
홈부분이 어떤걸 의미하는지 장소를 검색하는지 음악을 검색하는건지 애매한거같습니다.

- 사용하면서 불필요하게 보인 부분이 있었나요?
- 적용된 디자인의 좋은 점이나 개선이 필요한 디자인이 있나요?
    시간에 따라 핀의 색이 달라지면 좋겠음
    검색바 색상
    - 색상은 어떤가요?
            - 보라색의 채도가 너무 강해서 조금만 낮췄으면 좋겠습니다.
    - 폰트 크기는 적절한가요?
    - 아이콘은 명시적인가요?
- 이 앱을 누구에게 권장하고 싶나요? (새로운 페르소나를 구체적으로 제시해주세요)
이제 막 연애를 시작하는 사람
